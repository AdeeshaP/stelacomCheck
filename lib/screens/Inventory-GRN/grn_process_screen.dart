...
import 'package:stelacom_check/screens/Inventory-GRN/scan_verifying_screen.dart';
  ...
  void _navigateToIMEIScanner(dynamic item, String itemKey) async {
    ...
    if (expectedIMEIs.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'No expected IMEIs/Serial Numbers found for this item',
            style: TextStyle(
    ...
    if (actualQty <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Please enter actual quantity first',
            style: TextStyle(
              fontSize: Responsive.isMobileSmall(context)
                  ? 13
                  : Responsive.isMobileMedium(context) ||
    ...
    try {
      final result = await Navigator.push<List<String>>(
        context,
        MaterialPageRoute(
          builder: (context) => IMEIScannerScreen(
            item: item,
            itemKey: itemKey,
  ...
  void _completeGRN() {
    ...
    Future.delayed(Duration(seconds: 2), () {
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (context) {
            return TransferOrdersScreen(index: widget.index);
          },
        ),
